...
import 'package:nopsuite/Screens/Products/CompareProduct/Models/CompareProductModel.dart';
import 'package:nopsuite/Screens/Products/ProductEstimateShipping/Models/ProductEstimateShippingChangeRequestModel.dart';
import 'package:nopsuite/Screens/Products/ProductEstimateShipping/Models/ProductEstimateShippingModel.dart';
...
import 'package:nopsuite/Screens/Products/ProductReview/Models/GetProductReviewsModel.dart';
import 'package:nopsuite/Screens/General/LocalResourceProvider.dart';
import 'package:nopsuite/Screens/Products/ProductDetails/Model/ProductDetailsAttributeChangeModel.dart';
...
import 'package:nopsuite/Services/ProductDetailService.dart';
import 'package:nopsuite/Services/ShoppingCartService.dart';
import 'package:nopsuite/Services/WishlistService.dart';
...
class ProductDetailsProvider extends ChangeNotifier {
  ...
  String selectedTax = "";
  Map<int, ProductEstimateShippingChangeRequestModel>
  ...
  Map<int, ProductDetailsAttributeChangeModel>
      associateProductDetailsAttributeChangeModel = {};
  ...
  getProductDetails({required BuildContext context}) async {
    Response response = await ProductDetailsService().getProductDetails(
        context: context, params: "/$productId?updatecartitemid=$updateId");
    if (response.statusCode == 200) {
      ...
      if (getProductDetailsModel.productEstimateShipping.enabled) {
        if (getProductDetailsModel.productEstimateShipping.countryId == null ||
            getProductDetailsModel.productEstimateShipping.countryId == "") {
          List<CountryStateModel> countries = getProductDetailsModel
              .productEstimateShipping.availableCountries
          ...
          if (countries.isNotEmpty) {
            ...
            getProductDetailsModel.productEstimateShipping.countryId =
                getProductDetailsModel
                    .productEstimateShipping.availableCountries[0].value;
          }
        }

        if (getProductDetailsModel.productEstimateShipping.stateProvinceId ==
                null ||
            getProductDetailsModel.productEstimateShipping.stateProvinceId ==
                '') {
          List<CountryStateModel> states = getProductDetailsModel
              .productEstimateShipping.availableStates
              .where((element) => element.selected)
              .toList();
          if (states.isNotEmpty) {
            ...
            getProductDetailsModel.productEstimateShipping.stateProvinceId =
                getProductDetailsModel
                    .productEstimateShipping.availableStates[0].value;
          }
        }

        getProductDetailsModel.productEstimateShipping.city =
            getProductDetailsModel.productEstimateShipping.city == null
                ? ""
                : getProductDetailsModel.productEstimateShipping.city;
        getProductDetailsModel.productEstimateShipping.zipPostalCode =
            getProductDetailsModel.productEstimateShipping.zipPostalCode == null
                ? ""
                : getProductDetailsModel.productEstimateShipping.zipPostalCode;
      }
      ...
      if (getProductDetailsModel.addToCart.allowedQuantities.isNotEmpty) {
        ...
        List<AllowedQuantity> checkList = getProductDetailsModel
            .addToCart.allowedQuantities
            .where((element) =>
        ...
        if (checkList.isNotEmpty) {
          getProductDetailsModel.addToCart.enteredQuantity =
              int.parse(checkList[0].value);
        } else {
          getProductDetailsModel.addToCart.enteredQuantity = int.parse(
              getProductDetailsModel.addToCart.allowedQuantities[0].value);
        }
      }
      if (getProductDetailsModel.addToCart.customerEntersPrice) {
        getProductDetailsModel.addToCart.customerEnteredPrice =
            getProductDetailsModel.addToCart.customerEnteredPrice
                .ceil()
                .toDouble();
      }
  ...
  getProductReview({required BuildContext context}) async {
    Response response = await ProductDetailsService()
        .getProductReview(context: context, params: "/$productId");

    if (response.statusCode == 200) {
      // storage.setItem(
      //     StringConstants.CACHE_PRODUCT_DETAILS_REVIEW, response.body);
  ...
  getProductsAlsoPurchase({required BuildContext context}) async {
    Response response = await ProductDetailsService()
        .getProductsAlsoPurchase(context: context, params: "/$productId");
    if (response.statusCode == 200) {
      // storage.setItem(
      //     StringConstants.CACHE_PRODUCT_ALSO_PURCHASE, response.body);
  ...
  getRelatedProducts({required BuildContext context}) async {
    Response response = await ProductDetailsService()
        .getRelatedProducts(context: context, params: "/$productId");
    if (response.statusCode == 200) {
    ...
    if (getProductDetailsModel.productEstimateShipping.enabled) {
      if (getProductDetailsModel.associatedProducts.isNotEmpty) {
        ...
        estimateShippingData(
            context: context, productId: getProductDetailsModel.id);
      }
    } else {
  ...
  estimateShippingData(
    ...
    List<CountryStateModel> availableCountries = getProductDetailsModel
        .productEstimateShipping.availableCountries
        .where((element) => element.selected)
        .toList();
    if (availableCountries.isNotEmpty) {
      ...
      estimateCountry = int.parse(getProductDetailsModel
          .productEstimateShipping.availableCountries[0].value);
      estimateCountryName = getProductDetailsModel
          .productEstimateShipping.availableCountries[0].text;
    }

    List<CountryStateModel> availableStates = getProductDetailsModel
        .productEstimateShipping.availableStates
        .where((element) => element.selected)
        .toList();
    if (availableStates.isNotEmpty) {
      ...
      estimateState = int.parse(getProductDetailsModel
          .productEstimateShipping.availableStates[0].value);
      estimateStateName = getProductDetailsModel
          .productEstimateShipping.availableStates[0].text;
    }
    ...
    Response response = await ProductDetailsService().postEstimateShipping(
        context: context, params: "/$productId", body: body);

    if (response.statusCode == 200) {
      productEstimateShippingModelMap[productId] =
          productEstimateShippingModelFromJson(response.body);
      notifyListeners();
      ...
      if (shippingOptions.isNotEmpty) {
        ...
        for (var i
            in productEstimateShippingModelMap[productId]!.shippingOptions) {
          if (i.selected) {
            name = i.name;
            date = i.deliveryDateFormat;
            price = i.price;
          } else {
            name = productEstimateShippingModelMap[productId]!
                .shippingOptions[0]
                .name;
            date = productEstimateShippingModelMap[productId]!
                .shippingOptions[0]
                .deliveryDateFormat;
            price = productEstimateShippingModelMap[productId]!
                .shippingOptions[0]
                .price;
          }
        }
      ...
      estimateShippingMap[productId] =
          ProductEstimateShippingChangeRequestModel(
              productId: productId,
              zipPostalCode: getProductDetailsModel
                          .productEstimateShipping.zipPostalCode ==
                      null
                  ? ""
                  : getProductDetailsModel
                      .productEstimateShipping.zipPostalCode,
              city: getProductDetailsModel.productEstimateShipping.city == null
                  ? ""
                  : getProductDetailsModel.productEstimateShipping.city,
              countryId: estimateCountry,
              stateId: estimateCountry,
              name: name,
              date: date,
              price: price,
              stateName: estimateStateName,
      ...
      if (await CheckConnectivity().checkInternet(context)) {
        for (var attr in getProductDetailsModel.productAttributes) {
          ...
          if (attr.attributeControlType == AttributeControlType.Checkboxes ||
            ...
            for (var item
                in attr.values.where((element) => element.customerEntersQty)) {
              productChangedAttributes[
                      '"product_attribute_${attr.id}_${item.id}_qty"'] =
                  '"${item.quantity}"';
            }
          } else {
        ...
        if (getProductDetailsModel.isRental) {
          productChangedAttributes[
                  '"rental_start_date_${getProductDetailsModel.id}"'] =
              '"${getProductDetailsModel.rentalStartDate}"';
          productChangedAttributes[
                  '"rental_end_date_${getProductDetailsModel.id}"'] =
              '"${getProductDetailsModel.rentalEndDate}"';
        }

        if (getProductDetailsModel.addToCart.customerEntersPrice) {
          if (getProductDetailsModel.addToCart.customerEnteredPrice == null) {
            productChangedAttributes[
                    '"addtocart_${getProductDetailsModel.id}.CustomerEnteredPrice"'] =
                '"0"';
          } else {
            productChangedAttributes[
                    '"addtocart_${getProductDetailsModel.id}.CustomerEnteredPrice"'] =
        ...
        if (getProductDetailsModel.giftCard.isGiftCard) {
          if (getProductDetailsModel.giftCard.recipientName == null) {
            productChangedAttributes[
                '"giftcard_${getProductDetailsModel.id}.RecipientName"'] = '""';
          } else {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.RecipientName"'] =
                '"${getProductDetailsModel.giftCard.recipientName}"';
          ...
          if (getProductDetailsModel.giftCard.recipientEmail == null) {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.RecipientEmail"'] =
                '""';
          } else {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.RecipientEmail"'] =
          ...
          if (getProductDetailsModel.giftCard.senderName == null) {
            productChangedAttributes[
                '"giftcard_${getProductDetailsModel.id}.SenderName"'] = '""';
          } else {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.SenderName"'] =
                '"${getProductDetailsModel.giftCard.senderName}"';
          ...
          if (getProductDetailsModel.giftCard.senderEmail == null) {
            productChangedAttributes[
                '"giftcard_${getProductDetailsModel.id}.SenderEmail"'] = '""';
          } else {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.SenderEmail"'] =
                '"${getProductDetailsModel.giftCard.senderEmail}"';
          ...
          if (getProductDetailsModel.giftCard.message == null) {
            productChangedAttributes[
                '"giftcard_${getProductDetailsModel.id}.Message"'] = '""';
          } else {
            productChangedAttributes[
                    '"giftcard_${getProductDetailsModel.id}.Message"'] =
                '"${getProductDetailsModel.giftCard.message}"';
        ...
        Response response = await ProductDetailsService().postAttributeChange(
            context: context, params: "/$productId", body: body);
        if (response.statusCode == 200) {
          associateProductDetailsAttributeChangeModel[productId] =
              productDetailsAttributeChangeModelFromJson(response.body);
          if (productDetailsAttributeChangeModelFromJson(response.body).gtin !=
                  null &&
              productDetailsAttributeChangeModelFromJson(response.body).gtin !=
                  "") {
            getProductDetailsModel.gtin =
                productDetailsAttributeChangeModelFromJson(response.body).gtin;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body).sku !=
                  null &&
              productDetailsAttributeChangeModelFromJson(response.body).sku !=
                  "") {
            getProductDetailsModel.sku =
                productDetailsAttributeChangeModelFromJson(response.body).sku;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body).price !=
                  null &&
              productDetailsAttributeChangeModelFromJson(response.body).price !=
                  "") {
            getProductDetailsModel.productPrice.price =
                productDetailsAttributeChangeModelFromJson(response.body).price;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body)
                      .basepricepangv !=
                  null &&
              productDetailsAttributeChangeModelFromJson(response.body)
                      .basepricepangv !=
                  "") {
            getProductDetailsModel.productPrice.basePricePAngV =
                productDetailsAttributeChangeModelFromJson(response.body)
                    .basepricepangv;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body)
              ...
              productDetailsAttributeChangeModelFromJson(response.body)
                      .stockAvailability !=
                  "") {
            getProductDetailsModel.stockAvailability =
                productDetailsAttributeChangeModelFromJson(response.body)
                    .stockAvailability;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body)
              ...
              productDetailsAttributeChangeModelFromJson(response.body)
                      .pictureDefaultSizeUrl !=
                  "") {
            getProductDetailsModel.defaultPictureModel.imageUrl =
                productDetailsAttributeChangeModelFromJson(response.body)
                    .pictureDefaultSizeUrl;
          }
          if (productDetailsAttributeChangeModelFromJson(response.body)
              ...
              productDetailsAttributeChangeModelFromJson(response.body)
                      .isFreeShipping !=
                  "") {
            getProductDetailsModel.isFreeShipping =
                productDetailsAttributeChangeModelFromJson(response.body)
                    .isFreeShipping;
          }
        }
          ...
          for (var product in getProductDetailsModel.associatedProducts) {
            for (var attr in product.productAttributes) {
              ...
              if (attr.attributeControlType ==
                ...
                for (var item in attr.values
                    .where((element) => element.customerEntersQty)) {
                  productChangedAttributes[
                          '"product_attribute_${attr.id}_${item.id}_qty"'] =
                      '"${item.quantity}"';
                }
            ...
            if (product.isRental) {
              productChangedAttributes['"rental_start_date_${product.id}"'] =
                  '"${product.rentalStartDate}"';
              productChangedAttributes['"rental_end_date_${product.id}"'] =
                  '"${product.rentalEndDate}"';
            }

            if (product.addToCart.customerEntersPrice) {
              productChangedAttributes[
                      '"addtocart_${product.id}.CustomerEnteredPrice"'] =
                  '"${product.addToCart.customerEnteredPrice}"';
            }
            if (product.giftCard.isGiftCard) {
              productChangedAttributes[
                      '"giftcard_${product.id}.RecipientName"'] =
                  '"${product.giftCard.recipientName}"';
              productChangedAttributes[
                      '"giftcard_${product.id}.RecipientEmail"'] =
                  '"${product.giftCard.recipientEmail}"';
              productChangedAttributes['"giftcard_${product.id}.SenderName"'] =
                  '"${product.giftCard.senderName}"';
              productChangedAttributes['"giftcard_${product.id}.SenderEmail"'] =
                  '"${product.giftCard.senderEmail}"';
              productChangedAttributes['"giftcard_${product.id}.Message"'] =
                  '"${product.giftCard.message}"';
            }
            ...
            Response response = await ProductDetailsService()
                .postAttributeChange(
                    context: context, params: "/${product.id}", body: body);
            if (response.statusCode == 200) {
              associateProductDetailsAttributeChangeModel[product.id] =
                  productDetailsAttributeChangeModelFromJson(response.body);
  ...
  attributeOnChanged(
      {required BuildContext context,
      required String addToCartType,
      required GetProductDetailsModel getProductDetailsModel}) async {
    isAPILoader = true;
    notifyListeners();
    ...
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      Response response = await ProductDetailsService().postAttributeChange(
          context: context,
          params: "/${getProductDetailsModel.id}",
          body: body);
      if (response.statusCode == 200) {
        associateProductDetailsAttributeChangeModel[getProductDetailsModel.id] =
            productDetailsAttributeChangeModelFromJson(response.body);
        getProductDetailsModel.gtin =
            productDetailsAttributeChangeModelFromJson(response.body).gtin;
        getProductDetailsModel.sku =
    ...
    if (addToCartType == AddToCartType.Cart) {
      ...
      Response response = await ShoppingCartService()
          .postProductDetailsAddToCart(
              context: context,
              params: "/${getProductDetailsModel.id}",
              body: body);

      if (response.statusCode == 200) {
        FlushBarMessage().successMessage(
            context: context,
            title: LocalResourceProvider()
                .getResourseByKey('products.productHasBeenAddedToTheCart'));
      } else if (response.statusCode == 400) {
        InvalidResponseModel invalidResponseModel =
            invalidResponseModelFromJson(response.body);
        FlushBarMessage().failedMessage(
            context: context, title: invalidResponseModel.errors.join("\n"));
      }
      ...
      Response response = await WishlistService().productDetailsAddToWishlist(
          context: context,
          params: "/${getProductDetailsModel.id}",
          body: body);
      if (response.statusCode == 200) {
        ...
        FlushBarMessage().successMessage(
            context: context,
            title: LocalResourceProvider()
                .getResourseByKey('products.productHasBeenAddedToTheWishlist'));
      } else if (response.statusCode == 400) {
        InvalidResponseModel invalidResponseModel =
            invalidResponseModelFromJson(response.body);
        FlushBarMessage().failedMessage(
  ...
  downloadSample(
    ...
    if (permissionGranted) {
      ...
      Response response = await ProductDetailsService()
          .getDownloadProduct(context: context, params: "/$productId");
      if (response.statusCode == 200) {
  ...
  attributeUploadFile(
      {required BuildContext context,
      required ProductAttribute productAttribute,
      required var base64,
    ...
    Response response = await ProductDetailsService()
        .postUploadFileAttributeChange(
            context: context, params: "/${productAttribute.id}", body: body);
    if (response.statusCode == 200) {
  ...
  backInStockClickEvent({required BuildContext context}) async {
    ...
    Response response = await ProductDetailsService()
        .getBackInStockSubscribe(context: context, params: "/$productId");
    if (response.statusCode == 200) {
      getSubscribesModel = getSubscribesModelFromJson(response.body);
      max = LocalResourceProvider()
          .getResourseByKey("backInStockSubscriptions.maxSubscriptions");
      max = max.replaceAll(
          '{0}', getSubscribesModel.maximumBackInStockSubscriptions.toString());
    }
  ...
  backInStockSubscribe({required BuildContext context}) async {
    ...
    Response response = await ProductDetailsService()
        .postSubscription(context: context, params: "/$productId");
    if (response.statusCode == 200) {}
  ...
  addToCompareOnClickEvent(
      {required BuildContext context, required int productId}) {
    SQFLiteDatabase().dbHelper.addCompareProduct(
        model: CompareProductResourseModel(productId: productId));
    FlushBarMessage().successMessage(
        context: context,
        title: LocalResourceProvider()
            .getResourseByKey("products.producthasbeenaddedtocomparelist"));
  }

  updateEstimateData(
      {required BuildContext context,
      required GetProductDetailsModel getProductDetailsModel,
      required ProductEstimateShippingChangeRequestModel requestModel}) {
    getProductDetailsModel.productEstimateShipping.countryId =
        requestModel.countryId.toString();
    getProductDetailsModel.productEstimateShipping.zipPostalCode =
        requestModel.zipPostalCode;
    getProductDetailsModel.productEstimateShipping.city = requestModel.city;
    getProductDetailsModel.productEstimateShipping.stateProvinceId =
        requestModel.stateId.toString();
    estimateShippingMap[getProductDetailsModel.id] =
        ProductEstimateShippingChangeRequestModel(
            productId: productId,
  ...
  loadCacheData(
    ...
    if (this.productId == productId) {
      ...
      uploadFileModel = new UploadFileModel(
          success: false, uploadedFileGuid: "", message: "");
      isDownloaded = false;
      estimateShippingMap = {};
      isPageLoader = true;
      this.productId = productId;
