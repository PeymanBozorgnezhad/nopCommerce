...
import 'package:nopsuite/Screens/Products/EmailProduct/Models/GetProductEmailAFriendResponseModel.dart';
...
class EmailProductProvider extends ChangeNotifier {
  ...
  getEmailProduct({required BuildContext context}) async {
    Response response = await ProductService()
        .getEmailProduct(context: context, params: "/$productID");
    if (response.statusCode == 200) {
      getProductEmailAFriendResponseModel =
          getProductEmailAFriendResponseModelFromJson(response.body);
      isAPILoader = false;
      if (getProductEmailAFriendResponseModel.friendEmail != null) {
        friendEmailController.text =
            getProductEmailAFriendResponseModel.friendEmail;
      }

      if (getProductEmailAFriendResponseModel.yourEmailAddress != null) {
        yourEmailController.text =
            getProductEmailAFriendResponseModel.yourEmailAddress;
      }

      if (getProductEmailAFriendResponseModel.personalMessage != null) {
        messageController.text =
            getProductEmailAFriendResponseModel.personalMessage;
      }
      notifyListeners();
    ...
    if (response.statusCode == 200) {
      ...
      if (headerModel.alertMessage != "") {
        DialogBoxWidget().confirmationDialogBox(
            context: context,
            title:
                LocalResourceProvider().getResourseByKey("common.notification"),
  ...
  submitOnClickEvent({required BuildContext context}) async {
    ...
    Response response = await ProductService().postProductEmailAFriend(
    ...
    if (response.statusCode == 200) {
      ...
      FlushBarMessage().successMessage(
          context: context,
          title: response.body.toString().replaceAll('"', ""));
