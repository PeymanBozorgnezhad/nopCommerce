...
class ProductReviewProvider extends ChangeNotifier {
  ...
  SetProductReviewHelpfulnessModel setProductReviewHelpfulnessModel =
      new SetProductReviewHelpfulnessModel(result: '', totalYes: 0, totalNo: 0);
  ...
  pageLoadData(
      ...
      required ProductReviewOverview productReviewOverview}) async {
  ...
  getProductReview({required BuildContext context}) async {
    Response response = await ProductService().getProductReview(
        context: context, params: "/${productReviewOverview.productId}");
  ...
  getHeaderData({required BuildContext context}) async {
    ...
    if (response.statusCode == 200) {
      ...
      isHeaderLoader = false;
      isPageLoader = false;
      // storage.setItem(StringConstants.CACHE_HEADER, 'HeaderData');
      notifyListeners();
    }
    notifyListeners();
  }
  ...
  submitReview({required BuildContext context}) async {
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      Response response = await ProductService().addProductReview(
          context: context,
          params: "/${productReviewOverview.productId}",
          body: body);
      if (response.statusCode == 200) {
        getProductReview(context: context);
        ...
        FlushBarMessage().successMessage(
            context: context,
            title: StringConstants.PRODUCT_REVIEW_SUCCESS_MESSAGE);
  ...
  reviewHelpfulnessClick(
    ...
    Response response = await ProductService()
        .setProductReviewHelpfulness(context: context, body: body);
    if (response.statusCode == 200) {
      isAPILoader = false;
      setProductReviewHelpfulnessModel =
          setProductReviewHelpfulnessModelFromJson(response.body);
      notifyListeners();
      return true;
    } else {
      isAPILoader = false;
