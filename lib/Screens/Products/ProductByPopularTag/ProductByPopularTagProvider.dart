...
import 'package:nopsuite/Models/GetCatalogRootModel.dart';
import 'package:nopsuite/Models/ProductBoxModel.dart';
import 'package:nopsuite/Screens/Products/ProductDetails/Model/GetProductsByTagModel.dart';
...
import 'package:nopsuite/Services/ProductByCategoryService.dart';
...
class ProductByPopularTagProvider extends ChangeNotifier {
  ...
  getProductTagData(
      ...
      required String filterAttributes}) async {
    isAPILoader = true;
    notifyListeners();
    Response response = await ProductTagService().getProductByPopularTag(
        context: context,
        param:
            '/$tagId?Price=$minPrice-$maxPrice&OrderBy=$orderBy&PageSize=$pageSize&PageNumber=$pageNumber&specs=$filterAttributes');
    if (response.statusCode == 200) {
      ...
      sortList.clear();
      for (var i
          in getProductsByTagModel.catalogProductsModel.availableSortOptions) {
        if (selectedSort == "") {
          if (i.selected) {
            selectedSort = i.text;
            List<AvailableSortOption> list = getProductsByTagModel
                .catalogProductsModel.availableSortOptions
                .where((element) => element.text == selectedSort)
                .toList();
            getProductsByTagModel.catalogProductsModel.orderBy =
                int.parse(list[0].value);
          }
        }
        sortList.add(DropDownModel(
          text: i.text,
          value: i.value,
        ));
      }
      page.clear();
      for (var i
          in getProductsByTagModel.catalogProductsModel.pageSizeOptions) {
        if (i.selected == true) {
          pageSize = int.parse(i.text);
          selectedPage = pageSize;
        }
        page.add(DropDownModel(
          text: i.text,
          value: i.value,
        ));
      }
      ...
      if (getProductsByTagModel
          .catalogProductsModel.specificationFilter.enabled) {
        filterByAttributes.clear();
        for (var i in getProductsByTagModel
  ...
  getPopularProductTagsData({required BuildContext context}) async {
    Response response =
        await ProductTagService().getProductTag(context: context);
    if (response.statusCode == 200) {
      getPopularProductTagsModel =
          getPopularProductTagsModelFromJson(response.body);
    }
  ...
  getCatalogRoot({required BuildContext context}) async {
    Response response =
        await ProductByCategoryService().catalogRoot(context: context);
    if (response.statusCode == 200) {
  ...
  doneOnclickEvent({required BuildContext context}) async {
    ...
    filterAttributesDrawer =
        filterAttributesDrawer.substring(1, filterAttributesDrawer.length - 1);
    pageNumber = 1;
    ...
    if (maxPrice != 0) {
      getProductTagData(
          ...
          minPrice: minPrice,
          maxPrice: maxPrice,
          orderBy: getProductsByTagModel.catalogProductsModel.orderBy,
          pageSize: getProductsByTagModel.catalogProductsModel.pageSize,
          pageNumber: pageNumber,
          filterAttributes: filterAttributesDrawer);
    } else {
      FlushBarMessage().failedMessage(
          context: context,
          title: StringConstants.INVALID_SLIDER_PRICE_MESSAGE);
      isAPILoader = false;
    }
    notifyListeners();
  }

  filterAttributesOnclickEvent(
  ...
  Future loadMoreProduct({required BuildContext context}) async {
    if (getProductsByTagModel.catalogProductsModel.pageNumber !=
        getProductsByTagModel.catalogProductsModel.totalPages) {
      isLazyLoader = true;
      notifyListeners();
      Response response = await ProductTagService().getProductByPopularTag(
          context: context,
          param:
              '/$tagId?Price=$minPrice-$maxPrice&OrderBy=${getProductsByTagModel.catalogProductsModel.orderBy}&PageSize=${getProductsByTagModel.catalogProductsModel.pageSize}&PageNumber=${getProductsByTagModel.catalogProductsModel.pageNumber + 1}&specs=$filterByAttributes');
      if (response.statusCode == 200) {
        getProductsByTagModel = getProductsByTagModelFromJson(response.body);
      }
      products.addAll(getProductsByTagModel.catalogProductsModel.products);
