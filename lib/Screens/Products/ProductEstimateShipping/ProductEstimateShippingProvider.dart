...
import 'package:nopsuite/Services/ProductServices.dart';
...
class ProductEstimateShippingProvider extends ChangeNotifier {
  ...
  initMethode({required BuildContext context}) async {
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      for (var item in getProductDetailsModel
      ...
      getPreLoadStatesByCountryId(
          context: context, countryId: int.parse(countryId));
    }
  }
  ...
  getPreLoadStatesByCountryId(
    ...
    if (response.statusCode == 200) {
      ...
      stateProvinceId = stateModelList
          .where((element) =>
              element.id.toString() ==
              getProductDetailsModel.productEstimateShipping.stateProvinceId)
          .first
  ...
  productEstimateShippingData({required BuildContext context}) async {
    ...
    if (getProductDetailsModel.productEstimateShipping.useCity) {
      ...
      if (getProductDetailsModel.productEstimateShipping.zipPostalCode !=
          null) {
    ...
    productEstimateShippingRequestModel = ProductEstimateShippingRequestModel(
        productId: getProductDetailsModel.id,
        zipPostalCode: zipPostalCodeController.text.toString(),
        city: cityController.text.toString(),
        stateId: int.parse(stateProvinceId),
        countyId: int.parse(countryId),
        attributes: {});
    isAPILoader = false;
    //isPageLoader = false;
    notifyListeners();
  ...
  getEstimateShippingData({required BuildContext context}) async {
    ...
    Response response = await ProductService().postProductEstimateShipping(
        context: context, params: "/${getProductDetailsModel.id}", body: body);
    if (response.statusCode == 200) {
      ...
      productEstimateShippingModel.shippingOptions
          .where((element) =>
  ...
  onChangeEvent({required BuildContext context}) {
    if (countryId == "0") {
      ...
      if (getProductDetailsModel.productEstimateShipping.useCity &&
        ...
        isEstimateShippingOption = false;
        notifyListeners();
      } else if (!getProductDetailsModel.productEstimateShipping.useCity &&
        ...
        ProductEstimateShippingChangeRequestModel requestModel =
            new ProductEstimateShippingChangeRequestModel(
                ...
                price: productEstimateShippingModel.shippingOptions
                    ...
                    .first
                    .price,
                stateName: stateDropDownList
                    .where((element) => element.value == stateProvinceId)
                    .first
                    .text,
                countryName: getProductDetailsModel
                    .productEstimateShipping.availableCountries
                    .where((element) => element.value == countryId)
                    .first
                    .text);
        notifyListeners();
