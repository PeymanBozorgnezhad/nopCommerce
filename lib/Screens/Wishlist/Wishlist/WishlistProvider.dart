...
class WishlistProvider extends ChangeNotifier {
  ...
  getWishlistData({required BuildContext context}) async {
    ...
    if (response.statusCode == 200) {
      ...
      wishlistShareURL =
          APIConstants.WISHLIST_SHARE_BASEURL + wishlistModel.customerGuid;
      notifyListeners();
    }
  ...
  addToCartWishlistItem(
    ...
    Response response = await WishlistService()
        .addToCartWishlistItem(context: context, body: body);
    if (response.statusCode == 200) {
      Navigator.push(
              context,
              PageTransition(
  ...
  deleteWishlistItem(
    ...
    Response response = await WishlistService()
        .deleteWishlistItem(context: context, body: body);
    if (response.statusCode == 200) {
      getWishlistData(context: context);
    } else if (response.statusCode == 400) {
      InvalidResponseModel invalidResponseModel =
