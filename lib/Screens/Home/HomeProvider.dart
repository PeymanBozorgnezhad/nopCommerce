...
import 'package:nopsuite/Models/AppliedDiscountsWithCode.dart';
import 'package:nopsuite/Models/AttributeValueModel.dart';
import 'package:nopsuite/Models/CountryStateModel.dart';
import 'package:nopsuite/Models/GetTopicBlockModel.dart';
import 'package:nopsuite/Models/GiftCardDataModel.dart';
import 'package:nopsuite/Models/OrderTotals.dart';
import 'package:nopsuite/Screens/Checkout/MultiPageCheckout/MultiPageCheckout.dart';
import 'package:nopsuite/Screens/Checkout/OnePageCheckout.dart';
import 'package:nopsuite/Screens/Products/ProductEstimateShipping/Models/ProductEstimateShippingChangeRequestModel.dart';
import 'package:nopsuite/Screens/Products/ProductEstimateShipping/Models/ProductEstimateShippingRequestModel.dart';
import 'package:nopsuite/Utils/Enum/LoginTypeAttribute.dart';
import 'package:nopsuite/Utils/Enum/CustomerAttributes.dart';
import 'package:nopsuite/Common/FileConfig.dart';
import 'package:nopsuite/Screens/Home/Models/HomeModel/NivoSliderModel.dart';
import 'package:nopsuite/Screens/Home/Models/ShoppingCartModel/CartEstimateShippingRequestModel.dart';
import 'package:nopsuite/Models/CurrencyResponseModel.dart';
import 'package:nopsuite/Screens/Home/Models/GetCustomerNavigationModel.dart';
import 'package:nopsuite/Models/GetLanguageModel.dart';
import 'package:nopsuite/Models/TaxTypeModel.dart';
import 'package:nopsuite/Models/CommonStateModel.dart';
import 'package:nopsuite/Models/StateModel.dart';
import 'package:nopsuite/Models/UploadFileModel.dart';
import 'package:nopsuite/Models/ProductBoxModel.dart';
import 'package:nopsuite/Screens/Home/Models/ShoppingCartModel/CartAttributeChangeResponseModel.dart';
import 'package:nopsuite/Utils/CacheDatabase.dart';
...
import 'package:nopsuite/Screens/Home/Models/GetTopMenuModel.dart';
import 'package:nopsuite/Models/CommonProductResponseModel.dart';
...
import 'package:nopsuite/Services/HomeService.dart';
import 'package:nopsuite/Services/ShoppingCartService.dart';
import 'package:nopsuite/Themes/Flexo/Screen/MainScreen/CategoryPage/CategoryPage.dart';
import 'package:nopsuite/Themes/Flexo/Screen/MainScreen/HomePage/HomePage.dart';
import 'package:nopsuite/Themes/Flexo/Screen/MainScreen/ProfilePage/ProfilePage.dart';
import 'package:nopsuite/Themes/Flexo/Screen/MainScreen/ShoppingCartPage/ShoppingCart.dart';
...
class HomeProvider extends ChangeNotifier {
  ...
  late CartFilterEstimateShippingResponseModel
      cartFilterEstimateShippingResponseModel;
  ProductEstimateShippingChangeRequestModel preEstimateShipping =
      new ProductEstimateShippingChangeRequestModel(
          zipPostalCode: "",
          city: "",
          stateId: 0,
          countryId: 0,
  ...
  UploadFileModel uploadFileModel =
      new UploadFileModel(success: false, uploadedFileGuid: "", message: "");
  ...
  readConditionBox(
    ...
    DialogBoxWidget().informationDialogBox(
        context: context,
        title:
            LocalResourceProvider().getResourseByKey("checkout.termsOfService"),
        body: body,
        heading: title);
  }
  ...
  loadCacheData({required BuildContext context}) async {
    ...
    if (cacheNivoSlider != null) {
      ...
      nivoSliderModel.picture1Url.isNotEmpty
          ? nivoSlider.add(NivoSlider(
              nivoSliderModel.picture1Url,
              nivoSliderModel.link1,
              nivoSliderModel.text1,
              nivoSliderModel.altText1))
          : null;
      nivoSliderModel.picture2Url.isNotEmpty
          ? nivoSlider.add(NivoSlider(
              nivoSliderModel.picture2Url,
              nivoSliderModel.link2,
              nivoSliderModel.text2,
              nivoSliderModel.altText2))
          : null;
      nivoSliderModel.picture3Url.isNotEmpty
          ? nivoSlider.add(NivoSlider(
              nivoSliderModel.picture3Url,
              nivoSliderModel.link3,
              nivoSliderModel.text3,
              nivoSliderModel.altText3))
          : null;
      nivoSliderModel.picture4Url.isNotEmpty
          ? nivoSlider.add(NivoSlider(
              nivoSliderModel.picture4Url,
              nivoSliderModel.link4,
              nivoSliderModel.text4,
              nivoSliderModel.altText4))
          : null;
      nivoSliderModel.picture5Url.isNotEmpty
          ? nivoSlider.add(NivoSlider(
              nivoSliderModel.picture5Url,
              nivoSliderModel.link5,
              nivoSliderModel.text5,
              nivoSliderModel.altText5))
          : null;
    ...
    if (cacheLanguage != null) {
      ...
      preferences.setInt(SharedPreferencesValues.SHARED_PREFERENCE_LANGUAGE_ID,
          getLanguagesModel.currentLanguageId);
    }
  ...
  pageLoadData({required BuildContext context}) async {
    ...
    if (response.statusCode == 200) {
      ...
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_NIVO_SLIDER, 'NivoSlider');
      // });
    }
    isNivoLoader = false;
    notifyListeners();
  ...
  getHomeCategories({required BuildContext context}) async {
    Response response =
        await HomeService().getHomeCategoryData(context: context);
    if (response.statusCode == 200) {
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_HOME_CATEGORY, response.body);
      // });
      getHomepageCategoriesModel =
          getHomepageCategoriesModelFromJson(response.body);
      isHomePageCategoryLoader = false;
    } else {
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_HOME_CATEGORY, 'HomeCategory');
      // });
    }
    notifyListeners();
  ...
  getHomepageProduct({required BuildContext context}) async {
    Response response =
        await HomeService().getHomeProductsData(context: context);
    if (response.statusCode == 200) {
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_HOME_PRODUCTS, response.body);
      // });
      getHomepageProductsModel =
          commonProductResponseModelFromJson(response.body);
      isHomePageProductsLoader = false;
    } else {
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_HOME_PRODUCTS, 'HomeProducts');
      // });
    }
  ...
  getHomepageBestSeller({required BuildContext context}) async {
    Response response =
        await HomeService().getHomeBestSellerData(context: context);
    if (response.statusCode == 200) {
      // await storage.ready.then((_) {
      // storage.setItem(StringConstants.CACHE_HOME_BESTSELLER, response.body);
      // });
      getHomepageBestSellerModel =
          commonProductResponseModelFromJson(response.body);
  ...
  getHeaderData({required BuildContext context}) async {
    ...
    if (response.statusCode == 200) {
      ...
      if (headerModel.alertMessage != "") {
        DialogBoxWidget().confirmationDialogBox(
            context: context,
            title: "Notification",
  ...
  getCartData({required BuildContext context}) async {
    ...
    Response response =
        await ShoppingCartService().getCartData(context: context);
    if (response.statusCode == 200) {
      /* await storage.ready.then((_) {
  ...
  getCrossSellProducts({required BuildContext context}) async {
    Response response =
        await ShoppingCartService().getCrossProductCartData(context: context);
    if (response.statusCode == 200) {
      /* await storage.ready.then((_) {
  ...
  getTotalOrderData({required BuildContext context}) async {
    ...
    Response response =
        await ShoppingCartService().getCartTotalData(context: context);
    if (response.statusCode == 200) {
      /* await storage.ready.then((_) {
  ...
  getTopicBlockForHome({required BuildContext context}) async {
    Response response = await CommonService()
        .getTopicBlock(context: context, topic: "conditionsofuse");
    /* await storage.ready.then((_) {
  ...
  getEstimateShipping({required BuildContext context}) async {
    Response response = await ShoppingCartService()
        .getCartEstimateShippingData(context: context);
    if (response.statusCode == 200) {
      /* await storage.ready.then((_) {
        storage.setItem(
            StringConstants.CACHE_CART_ESTIMATE_SHIPPING, response.body);
      }); */
      estimateShippingResponseModel =
          estimateShippingResponseModelFromJson(response.body);
      isCartDataRefresh = false;
  ...
  estimateShippingData({required BuildContext context}) async {
    if (getCartModel.items.isNotEmpty) {
      if (estimateShippingResponseModel != null) {
        EstimateShippingResponseModel estimateShipping =
            estimateShippingResponseModel;
        List<CountryStateModel> availableCountries = estimateShipping
            .availableCountries
            .where((element) => element.selected)
            .toList();
        if (availableCountries.isNotEmpty) {
          preEstimateShipping.countryId =
              int.parse(availableCountries[0].value);
        } else {
          preEstimateShipping.countryId =
              int.parse(estimateShipping.availableCountries[0].value);
        }

        List<CountryStateModel> availableStates = estimateShipping
            .availableStates
            .where((element) => element.selected)
            .toList();
        if (availableStates.isNotEmpty) {
          ...
          preEstimateShipping.stateId =
              int.parse(estimateShipping.availableStates[0].value);
        }

        cartEstimateShippingRequestModel = CartEstimateShippingRequestModel(
            context: context,
            countryId: preEstimateShipping.countryId,
            stateId: preEstimateShipping.stateId,
            city: estimateShipping.city == null ? "" : estimateShipping.city,
            zipPostalCode: estimateShipping.zipPostalCode == null
                ? ""
        ...
        Response response = await ShoppingCartService()
            .postCartEstimateShippingData(context: context, body: body);
        if (response.statusCode == 200) {
          cartFilterEstimateShippingResponseModel =
              cartFilterEstimateShippingResponseModelFromJson(response.body);

          List<ShippingOption> shippingOptions =
              cartFilterEstimateShippingResponseModel.shippingOptions
                  .where((element) => element.selected)
                  .toList();
          String name = "";
          String price = "";
          ...
          if (shippingOptions.isNotEmpty) {
            ...
            name =
                cartFilterEstimateShippingResponseModel.shippingOptions[0].name;
            date = cartFilterEstimateShippingResponseModel
                .shippingOptions[0].deliveryDateFormat;
            price = cartFilterEstimateShippingResponseModel
                .shippingOptions[0].price;
          }

          preEstimateShipping = new ProductEstimateShippingChangeRequestModel(
              productId: 0,
              zipPostalCode: estimateShipping.zipPostalCode == null
                  ? ""
                  : estimateShipping.zipPostalCode,
  ...
  getNavigationData({required BuildContext context}) async {
    if (!isHeaderLoader) {
      if (headerModel.isAuthenticated) {
        Response response =
            await CommonService().getCustomerNavigation(context: context);
        if (response.statusCode == 200) {
          /* await storage.ready.then((_) {
            storage.setItem(StringConstants.CACHE_NAVIGATOR, response.body);
          }); */
          getCustomerNavigationModel =
              getCustomerNavigationModelFromJson(response.body);
          customerNavigationItems =
              getCustomerNavigationModel.customerNavigationItems;
          isProfileLoad = false;
          profileLoader = false;
  ...
  updateLanguage(
      {required BuildContext context, required String selectedValue}) async {
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      Response response = await CommonService()
          .postLanguages(context: context, params: "/$selectedValue");

      if (response.statusCode == 200) {
        SharedPreferences preferences = await SharedPreferences.getInstance();
        preferences.setInt(
            SharedPreferencesValues.SHARED_PREFERENCE_LANGUAGE_ID,
  ...
  updateCurrency(
    ...
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      await CommonService()
          .postCurrency(context: context, params: "/$selectedValue");
  ...
  updateTax({required BuildContext context, required String taxType}) async {
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      Response response = await CommonService()
          .postTaxType(context: context, params: "/$taxType");
      if (response.statusCode == 200) {
  ...
  removeCartItemClickEvent(
    ...
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      Response response = await ShoppingCartService()
          .deleteCartData(context: context, body: body);
      if (response.statusCode == 200) {
  ...
  updateCartClickEvent({required BuildContext context}) async {
    ...
    for (var item in getCartModel.items) {
      updateCartMap.add({'"cartItemId"': item.id, '"quantity"': item.quantity});
    }
    var body = '$updateCartMap';

    Response response = await ShoppingCartService()
        .updateCartQuantity(context: context, body: body);
  ...
  openFileExplorer(
      {required BuildContext context, required int attributeID}) async {
    bool permissionGranted = await FileConfig().permissionCheck(
        context: context,
        messageType: StringConstants.STORAGE_PERMISSION_MESSAGE);
  ...
  attributeUploadFile(
    ...
    final body = '{ '
        ...
        '}';
    Response response = await ShoppingCartService()
        .postCartUploadFileAttributeChange(
            context: context, body: body, params: "/$attributeID");
    if (response.statusCode == 200) {
      ...
      if (uploadFileModel.success) {
        ...
        FlushBarMessage()
            .successMessage(context: context, title: uploadFileModel.message);
      } else {
        FlushBarMessage()
            .failedMessage(context: context, title: uploadFileModel.message);
      }
    }
    removeShow = true;
    isCartDataRefresh = false;
  ...
  attributeOnChanged({required BuildContext context}) async {
    ...
    for (var i in getCartModel.checkoutAttributes) {
      List<String> idList = [];
      if (i.attributeControlType == AttributeControlType.Checkboxes ||
          i.attributeControlType == AttributeControlType.ReadonlyCheckboxes) {
        idList.clear();
    ...
    Response response = await ShoppingCartService().postCartAttributeChange(
        context: context, body: '$cartChangedAttributes');
    print("Att change ${response.statusCode}");
    if (response.statusCode == 200) {
      orderTotalResponseModel =
          cartAttributeChangeResponseModelFromJson(response.body).orderTotals;
  ...
  getStateDataByCountryId(
      ...
      required EstimateShippingResponseModel
          estimateShippingResponseModel}) async {
    stateModelList.clear();
    Response response = await CommonService()
        .getStateData(context: context, countryId: countryId);
    if (response.statusCode == 200) {
      stateModelList = getStatesModelFromJson(response.body);
      filterStateList.clear();
      for (var item in stateModelList) {
        filterStateList
            .add(new DropDownModel(text: item.name, value: item.name));
      }
    ...
    if (estimateShippingResponseModel != null) {
      ...
      for (var item in estimateShippingModel.availableCountries) {
        availableCountryList
            .add(new DropDownModel(text: item.text, value: item.text));
      }
  ...
  discountCouponClickEvent({required BuildContext context}) async {
    isDiscountApplied = false;
    discountCouponMsg = "";
    notifyListeners();
    ...
    if (discountCodeController.text.toString().isNotEmpty) {
      ...
      Response response = await ShoppingCartService().postDiscountCoupon(
          context: context,
          params:
              '/${discountCodeController.text.toString()}?prepareOrderTotals=true');
      if (response.statusCode == 200) {
        DiscountCouponResponseModel discountModel =
            discountCouponResponseModelFromJson(response.body);
      ...
      isCartDataRefresh = false;
      discountCouponMsg = LocalResourceProvider()
          .getResourseByKey("shoppingCart.discountCouponCode.empty");
      notifyListeners();
    }
  }
  ...
  getDiscountCartData(
      {required BuildContext context,
      required DiscountCouponResponseModel discountModel}) async {
    Response response =
        await ShoppingCartService().getCartData(context: context);
    if (response.statusCode == 200) {
      getCartModel.discountBox =
          getCartModelFromJson(response.body).discountBox;
  ...
  removeDiscountCouponClickEvent(
      {required BuildContext context,
      required AppliedDiscountsWithCode appliedDiscountsWithCode}) async {
    isDiscountApplied = false;
    discountCouponMsg = "";
    isCartDataRefresh = true;
    FocusScope.of(context).requestFocus(new FocusNode());
    ...
    Response response = await ShoppingCartService().removeDiscountCoupon(
        context: context,
        params:
            "/${appliedDiscountsWithCode.id.toString()}?prepareOrderTotals=true");
    if (response.statusCode == 200) {
      orderTotalResponseModel = orderTotalResponseModelFromJson(response.body);
      getCartModel.discountBox.appliedDiscountsWithCodes
          .removeWhere((element) => element.id == appliedDiscountsWithCode.id);
  ...
  giftCardCouponCodeClickEvent({required BuildContext context}) async {
    ...
    if (giftCardController.text.trim().toString().isNotEmpty) {
      Response response = await ShoppingCartService().postGiftCardCoupon(
          context: context,
          params:
              '/${giftCardController.text.trim().toString()}?prepareOrderTotals=true');
      if (response.statusCode == 200) {
      ...
      isGiftCardApplied = false;
      giftCardCouponMsg = LocalResourceProvider()
          .getResourseByKey("shoppingCart.discountCouponCode.empty");
      notifyListeners();
    }
  ...
  removeGiftCardClickEvent(
    ...
    Response response = await ShoppingCartService().removeGiftCardCoupon(
        context: context,
        params: "/${giftCardData.id.toString()}?prepareOrderTotals=true");
    if (response.statusCode == 200) {
      getTotalOrderData(context: context);
      notifyListeners();
    }
  ...
  downloadPDF({required BuildContext context}) async {
    ...
    if (permissionGranted) {
      ...
      Response response = await ShoppingCartService()
          .getPdfDownload(context: context, params: "/$fileGuid");
      if (response.statusCode == 200) {
    ...
    if (productsWarnings == 0) {
      ...
      for (var i in getCartModel.checkoutAttributes) {
        ...
        if (i.attributeControlType == AttributeControlType.Checkboxes ||
          ...
          startCheckoutMap['"$prefixString${i.id}_month"'] =
              '"${i.selectedMonth}"';
          startCheckoutMap['"$prefixString${i.id}_year"'] =
              '"${i.selectedYear}"';
        } else {
          if (i.defaultValue != null) {
            idList.add('"${i.defaultValue}"');
      ...
      Response response = await ShoppingCartService()
          .startCheckout(context: context, body: '$startCheckoutMap');
      if (response.statusCode == 200) {
        if (headerModel.isAuthenticated) {
          LocalResourceProvider().getSettingByName(
                      "orderSettings.onePageCheckoutEnabled") ==
                  'False'
              ? Navigator.push(
                      context,
                      PageTransition(
                          type: selectedTransition, child: MultiPageCheckout()))
                  .then((value) {
                  getCartData(context: context);
                  getHeaderData(context: context);
                })
              : Navigator.push(
                      context,
                      PageTransition(
                          type: selectedTransition, child: OnePageCheckout()))
                  .then((value) {
                  getCartData(context: context);
                  getHeaderData(context: context);
                });
          ...
          Navigator.push(
                  ...
                  PageTransition(
                      ...
                      child: Login(loginType: LoginTypeAttribute.Checkout)))
              .then((value) {
            pageLoadData(context: context);
          });
        }
  ...
  removeFileAttribute(
      {required BuildContext context, required int attributeId}) {
    removeShow = false;
  ...
  logoutUser({required BuildContext context}) async {
    ...
    /* LocalStorage storage =
        new LocalStorage(StringConstants.CACHE_DATABASE_NAME); */

    Response response = await CustomerService().logoutUser(context: context);
    print("Logout ${response.statusCode}");
    if (response.statusCode == 200) {
      preferences.setBool(
          SharedPreferencesValues.SHARED_PREFERENCE_IS_LOGIN, false);
      preferences.setBool(
          SharedPreferencesValues.SHARED_PREFERENCE_IS_WITHOUT_LOGIN, false);
      ...
      Navigator.pushReplacement(
          ...
          PageTransition(
              type: selectedTransition,
              child: Login(loginType: LoginTypeAttribute.Login)));
    } else {
      preferences.setBool(
          SharedPreferencesValues.SHARED_PREFERENCE_IS_LOGIN, false);
      preferences.setBool(
  ...
  getCacheCartEstimateShipping() async {
    /* await storage.ready.then((_) {
      String res =
          storage.getItem(StringConstants.CACHE_CART_ESTIMATE_SHIPPING);
      if (res != null) {
        estimateShippingResponseModel =
            estimateShippingResponseModelFromJson(res);
  ...
  getCacheLanguage() async {
    ...
    updateEstimateData(
        {required BuildContext context,
        required ProductEstimateShippingChangeRequestModel requestModel}) {
      ...
      preEstimateShipping = ProductEstimateShippingChangeRequestModel(
          productId: 0,
          zipPostalCode: requestModel.zipPostalCode,
    ...
    /* selectShippingOption(
        {required BuildContext context,
        required ProductEstimateShippingChangeRequestModel
      ...
      final body = '{ '
          ...
          '}';
      Response response = await ShoppingCartService().postSelectShippingOption(
          context: context, params: "/${shippingOption.name}", body: body);
      if (response.statusCode == 200) {
        orderTotalResponseModel =
            orderTotalResponseModelFromJson(response.body);
      }
      isCartDataRefresh = false;
      notifyListeners();
    } */
