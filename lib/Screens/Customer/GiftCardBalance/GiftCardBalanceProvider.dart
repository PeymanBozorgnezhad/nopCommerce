...
import 'package:nopsuite/Screens/Customer/GiftCardBalance/Models/GetGiftCardBalanceResponseModel.dart';
import 'package:nopsuite/Services/CommonService.dart';
import 'package:nopsuite/Services/GiftCardBalanceService.dart';
...
class GiftCardBalanceProvider extends ChangeNotifier {
  ...
  getGiftCard({required BuildContext context}) async {
    ...
    notifyListeners();
    Response response =
        await GiftCardBalanceService().getGiftCardBalance(context: context);
    if (response.statusCode == 200) {
      getGiftCardBalanceResponseModel =
          getGiftCardBalanceResponseModelFromJson(response.body);
  ...
  buttonClickEvent({required BuildContext context}) async {
    isAPILoader = true;
    notifyListeners();
    Response response = await GiftCardBalanceService().applyGiftCardBalance(
        context: context,
        param: "/${giftCodeController.text.trim().toString()}");
    if (response.statusCode == 200) {
      getGiftCardBalanceResponseModel =
          getGiftCardBalanceResponseModelFromJson(response.body);
      isAPILoader = false;
