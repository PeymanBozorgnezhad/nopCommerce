...
class RegisterProvider extends ChangeNotifier {
  ...
  getRegister({required BuildContext context}) async {
    Response response =
        await CustomerService().getRegisterData(context: context);
    if (response.statusCode == 200) {
      getRegisterResponseModel =
          getRegisterResponseModelFromJson(response.body);
  ...
  getPrivacyTopicBlock({required BuildContext context}) async {
    Response response = await CommonService()
        .getTopicBlock(context: context, topic: 'privacyInfo');
    if (response.statusCode == 200) {
  ...
  getData({required BuildContext context}) async {
    ...
    for (var c in getRegisterResponseModel.availableStates) {
      stateDropDown.add(DropDownModel(
        text: c.text,
        value: c.value,
      ));
    }
    ...
    genderData.clear();
    genderData.add(GenderData(
        id: 1,
        code: "M",
        name: LocalResourceProvider()
            .getResourseByKey("account.fields.gender.male")));
    genderData.add(GenderData(
        id: 2,
        code: "F",
        name: LocalResourceProvider()
            .getResourseByKey("account.fields.gender.female")));

    if (getRegisterResponseModel.countryEnabled) {
      if (getRegisterResponseModel.countryId == null ||
          getRegisterResponseModel.countryId == "") {
        getRegisterResponseModel.countryId =
            int.parse(getRegisterResponseModel.availableCountries[0].value);
      }
    }

    if (getRegisterResponseModel.stateProvinceEnabled &&
        getRegisterResponseModel.countryEnabled) {
      if (getRegisterResponseModel.stateProvinceId == null &&
          getRegisterResponseModel.stateProvinceId == "") {
        getRegisterResponseModel.stateProvinceId =
            int.parse(getRegisterResponseModel.availableStates[0].value);
      }
    }

    if (getRegisterResponseModel.allowCustomersToSetTimeZone) {
      if (getRegisterResponseModel.timeZoneId == null ||
          getRegisterResponseModel.timeZoneId == "") {
        getRegisterResponseModel.timeZoneId =
            getRegisterResponseModel.availableTimeZones[0].value;
      }
    }
  ...
  getStateDataByCountryId(
    ...
    if (await CheckConnectivity().checkInternet(context)) {
      ...
      for (var item in stateModelList) {
        stateDropDown
            .add(DropDownModel(text: item.name, value: item.id.toString()));
      }
  ...
  registerButtonClickEvent(BuildContext context, loginType) async {
    if (dateOfBirthController.text.trim().isNotEmpty) {
      List<String> datOfBirthList =
          dateOfBirthController.text.trim().toString().split("/");
    ...
    for (var item in getRegisterResponseModel.gdprConsents) {
      ...
      gdprConsentsSelected['"$consentsPrefixString${item.id}"'] =
          '"$checkCondition"';
    }
    ...
    Response response =
        await CustomerService().userRegister(context: context, body: body);
    ...
    if (response.statusCode == 400) {
      ...
      if (registerResponseModel.accessToken == null ||
          registerResponseModel.accessToken == "") {
        Navigator.pushReplacement(
            context,
            PageTransition(
                type: selectedTransition,
                child: Login(
                  loginType: LoginTypeAttribute.Login,
                )));
      } else {
        SharedPreferences preferences = await SharedPreferences.getInstance();
        preferences.setString(SharedPreferencesValues.SHARED_PREFERENCE_TOKEN,
            registerResponseModel.accessToken);
        preferences.setString(
            SharedPreferencesValues.SHARED_PREFERENCE_REFRESH_TOKEN,
            registerResponseModel.refreshToken);
        preferences.setBool(
            SharedPreferencesValues.SHARED_PREFERENCE_IS_LOGIN, true);
        preferences.setBool(
      ...
      FlushBarMessage().successMessage(
          context: context, title: registerResponseModel.message);
    }
    notifyListeners();
  }
  ...
  checkUserNameAvailability({required BuildContext context}) async {
    if (userNameController.text.isNotEmpty) {
      ...
      Response response = await CustomerService().checkAvailability(
      ...
      if (checkData != null) {
        ...
        if (checkData.toString().trim() ==
            LocalResourceProvider().getResourseByKey(
                "account.checkUserNameAvailability.available")) {
    ...
    } else {
      {
        FlushBarMessage().failedMessage(
            ...
            title: LocalResourceProvider()
                .getResourseByKey("account.fields.username.required"));
        checkAvailability = "";
      }
    }
    notifyListeners();
