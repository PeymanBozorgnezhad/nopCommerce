...
import 'package:nopsuite/Screens/Customer/GdprTools/Models/DeleteGdprToolsResponseModel.dart';
import 'package:nopsuite/Services/CommonService.dart';
import 'package:nopsuite/Services/GdprToolService.dart';
...
class GdprToolsProvider extends ChangeNotifier {
  ...
  deleteButtonClickEvent({required BuildContext context}) async {
    ...
    notifyListeners();
    Response response =
        await GdprToolService().deleteGdprTool(context: context);
    if (response.statusCode == 200) {
      deleteGdprToolsResponseModel =
          deleteGdprToolsResponseModelFromJson(response.body);
      isAPILoader = false;
      FlushBarMessage().successMessage(
          context: context, title: deleteGdprToolsResponseModel.result);
    }
    notifyListeners();
  }
}
