...
import 'package:nopsuite/Screens/Customer/Order/OrderList/Model/CustomerOrderModel.dart';
...
class OrderListProvider extends ChangeNotifier {
  ...
  getCustomerOrderList({required BuildContext context}) async {
    Response response =
        await OrderServices().getListOfOrderData(context: context);
  ...
  cancelOnClickEvent(
      {required BuildContext context, required int recurringPaymentId}) async {
    isAPILoader = true;
    notifyListeners();
    Response response = await OrderServices().cancelRecurringPayment(
        context: context, param: '/$recurringPaymentId');
    if (response.statusCode == 200) {
      customerOrderModel = customerOrdersModelFromJson(response.body);
  ...
  retryLastRecurringPayment(
      ...
      required int recurringPaymentId,
    ...
    Response response = await OrderServices().retryLastRecurringPayment(
        context: context, param: '/$recurringPaymentId', body: body);
    if (response.statusCode == 200) {
      customerOrderModel = customerOrdersModelFromJson(response.body);
      await getCustomerOrderList(context: context);
      isAPILoader = false;
