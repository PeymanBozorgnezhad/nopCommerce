...
import 'package:nopsuite/Screens/Customer/RewardPoint/Model/GetCustomerRewardPointModel.dart';
import 'package:nopsuite/Services/CommonService.dart';
import 'package:nopsuite/Services/RewardPointService.dart';
...
class RewardPointProvider extends ChangeNotifier {
  ...
  getCustomerRewardPoints({required BuildContext context}) async {
    Response response = await RewardPointService().customerRewardPoint(
        context: context, param: '?pageNumber=$pageNumber');
    if (response.statusCode == 200) {
      // storage.setItem(StringConstants.CACHE_REWARD_POINT, response.body);
      getCustomerRewardPointsModel =
          getCustomerRewardPointsModelFromJson(response.body);
  ...
  Future loadMoreRewardsPoints({required BuildContext context}) async {
    if (getCustomerRewardPointsModel.pagerModel.currentPage !=
        getCustomerRewardPointsModel.pagerModel.totalPages) {
      isLazyLoader = true;
      notifyListeners();
      Response response = await RewardPointService().customerRewardPoint(
          context: context,
          param:
              '?pageNumber=${getCustomerRewardPointsModel.pagerModel.currentPage + 1}');
      if (response.statusCode == 200) {
        getCustomerRewardPointsModel =
            getCustomerRewardPointsModelFromJson(response.body);
        rewardPoints.addAll(getCustomerRewardPointsModel.rewardPoints);
