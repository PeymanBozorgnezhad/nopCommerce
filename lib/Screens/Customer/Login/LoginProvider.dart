...
class LoginProvider extends ChangeNotifier {
  ...
  guestToken({required BuildContext context}) async {
    ...
    if (response.statusCode == 200) {
      ...
      preferences.setString(
          SharedPreferencesValues.SHARED_PREFERENCE_TOKEN, token);
  ...
  getTopicBlock({required BuildContext context}) async {
    Response response = await CommonService()
        .getTopicBlock(context: context, topic: 'CheckoutAsGuestOrRegister');
    if (response.statusCode == 200) {
  ...
  loginClick({required BuildContext context}) async {
    ...
    Response response =
        await CustomerService().userLogin(context: context, body: body);
    ...
    if (response.statusCode == 200) {
      ...
      preferences.setString(
          SharedPreferencesValues.SHARED_PREFERENCE_TOKEN_EXPIRATION_DATE,
          loginResponseModel.refreshTokenExpiration.toString());
      preferences.setBool(
