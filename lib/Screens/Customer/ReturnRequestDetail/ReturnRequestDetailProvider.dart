...
import 'package:nopsuite/Screens/Customer/ReturnRequestDetail/Model/GetReturnRequestModel.dart';
import 'package:nopsuite/Screens/Customer/ReturnRequestDetail/Model/ReturnRequestModel.dart';
import 'package:nopsuite/Models/UploadFileModel.dart';
import 'package:nopsuite/Screens/Customer/ReturnRequestDetail/ReturnRequestMassage.dart';
import 'package:nopsuite/Services/CommonService.dart';
import 'package:nopsuite/Services/ReturnRequestService.dart';
...
class ReturnRequestDetailProvider extends ChangeNotifier {
  ...
  getReturnRequestByOrderId(
    ...
    Response response = await ReturnRequestService()
        .getReturnRequest(context: context, param: "/$orderId");
    if (response.statusCode == 200) {
      ...
      for (var Items in getReturnRequestModel.availableReturnReasons) {
        reasonDropdown
            .add(new DropDownModel(text: Items.name, value: Items.name));
      }
      ...
      if (paths != null && paths != "") {
        ...
        if (base64.isNotEmpty) {
          ...
          final body = '{ '
              ...
              '}';
          Response response = await ReturnRequestService()
              .uploadFileReturnRequest(context: context, body: body);
          if (response.statusCode == 200) {
            uploadFileModel = uploadFileModelFromJson(response.body);
            getReturnRequestModel.uploadedFileGuid =
                uploadFileModel.uploadedFileGuid;
            if (uploadFileModel.success) {
              FlushBarMessage().successMessage(
                  context: context, title: uploadFileModel.message);
            }
            isAPILoader = false;
          }
        }
  ...
  updateButtonClickEvent(
    ...
    Response response = await ReturnRequestService()
        .returnRequest(context: context, body: body, param: '/$orderId');
    if (response.statusCode == 200) {
      ...
      Navigator.push(
          ...
          PageTransition(
              ...
              child: ReturnRequestMassage(returnRequestModel.result,
                  returnRequestModel.customOrderNumber)));
