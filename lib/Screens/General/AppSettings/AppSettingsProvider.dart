...
class AppSettingsProvider extends ChangeNotifier {
  ...
  void getAllLocalResourseLanguages(
      {required BuildContext context, required StateSetter setState}) async {
    SQFLiteDatabase().dbHelper.deleteAll();
    Response response =
        await CommonService().getLocalResource(context: context);
    if (response.statusCode == 200) {
      List<LocalStringResourceModel> localStringResourseModel =
          localStringResourseModelFromJson(response.body);
      ...
      preferences.setBool(
          SharedPreferencesValues.SHARED_PREFERENCE_IS_LOAD_LOCAL_DATA, true);
      setState(() {
  ...
  Future<bool> handleWillPop(BuildContext context) async {
    FlushBarMessage().warningMessage(
        context: context, title: "Please wait while downloading data");
    return false;
  }
}
