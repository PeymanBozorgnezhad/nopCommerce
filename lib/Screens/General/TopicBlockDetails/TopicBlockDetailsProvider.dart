...
class TopicBlockDetailsProvider extends ChangeNotifier {
  ...
  pageLoadData(
      ...
      required String topicBody}) async {
    isPageLoader = true;
    isAPILoader = false;
  ...
  pageLoadDataById(
      ...
      required String topicType}) async {
    isPageLoader = true;
    this.name = name;
    this.seName = seName;
  ...
  getTopicBlocData({required BuildContext context}) async {
    ...
    if (topicType == TopicType.Id) {
      response = await CommonService()
          .getTopicBlockById(context: context, topicId: topicId);
    } else {
      response =
          await CommonService().getTopicBlock(context: context, topic: seName);
    }
