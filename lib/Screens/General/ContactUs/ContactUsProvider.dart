...
import 'package:nopsuite/Screens/General/ContactUs/Model/ContactUsModel.dart';
...
class ContactUsProvider extends ChangeNotifier {
  ...
  /* getCacheContactUs() async {
    ...
    if (res != null) {
      ...
      if (getContactUsModel.subject != null &&
          getContactUsModel.subjectEnabled) {
  ...
  getTopicBlock({required BuildContext context}) async {
    Response response = await CommonService()
        .getTopicBlock(context: context, topic: "ContactUs");
  ...
  submitContactUsData({required BuildContext context}) async {
    ...
    Response response =
        await CommonService().postContactUs(context: context, body: body);
    if (response.statusCode == 200) {
      isAPILoader = false;
      notifyListeners();
      ...
      FlushBarMessage().successMessage(
          ...
          title: LocalResourceProvider()
              .getResourseByKey("contactus.yourEnquiryHasBeenSent"));
