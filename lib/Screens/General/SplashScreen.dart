...
import 'package:nopsuite/Themes/Flexo/Screen/General/SplashScreen/SplashScreen.dart';
...
class _SplashScreenState extends State<SplashScreen> {
 ...
 inItFunction();
    ...
    //       SharedPreferencesValues.SHARED_PREFERENCE_CURRENT_VERSION,
    //       currentVersion);
    //   inItFunction();
    // } else {
    //   FlushBarMessage().failedMessage(
    //       context: context, title: StringConstants.ERROR_MESSAGE_500);
    // }
  }
  ...
  inItFunction() async {
    ...
    if (preferences
            .getInt(SharedPreferencesValues.SHARED_PREFERENCE_LANGUAGE_ID) ==
        null) {
      preferences.setInt(
          SharedPreferencesValues.SHARED_PREFERENCE_LANGUAGE_ID, 1);
    }

    // Timer(const Duration(seconds: 3), () {

    if (preferences.getBool(
                SharedPreferencesValues.SHARED_PREFERENCE_IS_LOAD_LOCAL_DATA) ==
            null ||
        preferences.getBool(
                SharedPreferencesValues.SHARED_PREFERENCE_IS_LOAD_LOCAL_DATA) !=
            true) {
      Navigator.push(context,
          PageTransition(type: selectedTransition, child: AppSettings()));
    } else {
      if (preferences.getBool(
                      SharedPreferencesValues.SHARED_PREFERENCE_IS_LOGIN) !=
                  null &&
              preferences.getBool(
                      SharedPreferencesValues.SHARED_PREFERENCE_IS_LOGIN) ==
                  true ||
          preferences.getBool(
                  SharedPreferencesValues.SHARED_PREFERENCE_IS_WITHOUT_LOGIN) ==
              true) {
