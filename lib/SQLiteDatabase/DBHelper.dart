...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    await db.execute(
        'CREATE TABLE LocalStringResourse (Id INTEGER PRIMARY KEY AUTOINCREMENT, LanguageId INTEGER, ResourceName TEXT, ResourceValue TEXT)');
    await db.execute(
        'CREATE TABLE AllSettings (Id INTEGER PRIMARY KEY AUTOINCREMENT, Name TEXT, Value TEXT)');
    await db.execute(
        'CREATE TABLE CompareProducts (Id INTEGER PRIMARY KEY AUTOINCREMENT, ProductId INTEGER)');
  }

  Future<LocalStringResourceModel> addResourse(
      {required LocalStringResourceModel model}) async {
    final Database db = await initDatabase();
    model.languageId = await db.insert('LocalStringResourse', model.toJson());
    return model;
  }

  Future<String> getResourseIdData(
      {required int languageId, required String key}) async {
    final Database db = await initDatabase();
    List<Map> maps = await db.rawQuery(
        'SELECT * FROM LocalStringResourse WHERE LanguageId=$languageId and ResourceName="' +
            key +
            '"');
    LocalStringResourceModel model = new LocalStringResourceModel(
        languageId: languageId, resourceName: key, resourceValue: key);
    String value = "";
    if (maps.length > 0) {
      ...
      model = new LocalStringResourceModel(
          languageId: languageId, resourceName: key, resourceValue: key);
      value = model.resourceValue;
    }
    return value;
  ...
  Future<List<ResourceModel>> getResourseLanguageIdData(
      {required int languageId}) async {
    final Database db = await initDatabase();
    List<Map> maps = await db.rawQuery(
        'SELECT * FROM LocalStringResourse WHERE LanguageId=$languageId');
    LocalStringResourceModel localModel = new LocalStringResourceModel(
        languageId: languageId, resourceName: "", resourceValue: "");
    List<ResourceModel> model = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        localModel = LocalStringResourceModel.fromJson(maps[i]);
        model.add(new ResourceModel(
            localModel.resourceName, localModel.resourceValue));
      }
    }
    return model;
  }
  ...
  Future<String> getSettingByName({required String name}) async {
    final Database db = await initDatabase();
    List<Map> maps =
        await db.rawQuery('SELECT * FROM AllSettings WHERE Name="$name"');
  ...
  //Compare Products
  Future<CompareProductResourseModel> addCompareProduct(
      {required CompareProductResourseModel model}) async {
    final Database db = await initDatabase();
  ...
  Future<List<CompareProductResourseModel>> getCompareProductsData() async {
    ...
    List<Map> maps = await db
        .rawQuery('SELECT ProductId FROM CompareProducts Group By ProductId');
    CompareProductResourseModel localModel =
        new CompareProductResourseModel(productId: 0);
    List<CompareProductResourseModel> model = [];
    if (maps.length > 0) {
      for (int i = 0; i < maps.length; i++) {
        localModel = CompareProductResourseModel.fromJson(maps[i]);
        model.add(
            new CompareProductResourseModel(productId: localModel.productId));
      }
    }
    return model;
  }
