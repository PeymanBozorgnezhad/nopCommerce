        ...
        return GestureDetector(
          ...
          child: Scaffold(
            ...
            bottomNavigationBar: FlexoBottomNavigationWidget().bottomNavigation(
                ...
                headerData: orderShipmateDetailProvider.headerModel,
                headerLoader: orderShipmateDetailProvider.isAPILoader),
            appBar: FlexoAppBarWidget().appbar(
                context: context,
                backButton: true,
                title: orderShipmateDetailProvider.isPageLoader
                    ? ""
                    : localResourceProvider
                        .getResourseByKey("order.shipments.information")
                        .toString()
                        .replaceAll("{0}",
                            " ${orderShipmateDetailProvider.getShipmentDetailsModel.id}")),
            body: orderShipmateDetailProvider.isPageLoader
                ? Loaders.pageLoader()
                : Column(
                    children: [
                      orderShipmateDetailProvider.isAPILoader
                          ? Loaders.apiLoader()
                          : Container(),
                      Expanded(
                        child: Container(
                          child: SingleChildScrollView(
                            child: Column(
                              children: [
                                OrderShipmateDetailsComponent()
                                    .getOrderShipmateDetails(context: context),
                              ],
                            ),
                          ),
                        ),
