    ...
    return Scaffold(
      appBar: FlexoAppBarWidget().appbar(
          ...
          title: localResourceProvider.isLocalDataLoad ||
                  productReviewProvider.isPageLoader
              ? ""
              : localResourceProvider.getResourseByKey(
                      "account.customerProductReviews.productReviewFor") +
                  " " +
                  productReviewProvider.getProductReviewsModel.productName,
          backButton: true),
      ...
      bottomNavigationBar: FlexoBottomNavigationWidget().bottomNavigation(
          ...
          headerData: productReviewProvider.headerModel,
          headerLoader: productReviewProvider.isPageLoader),
      body: productReviewProvider.isPageLoader ||
              localResourceProvider.isLocalDataLoad
          ? Loaders.pageLoader()
          : Column(
              children: [
                productReviewProvider.isAPILoader
                    ? Loaders.apiLoader()
                    : Container(),
                Expanded(
                  child: Container(
                    child: SingleChildScrollView(
                      child: Column(
                        children: [
                          productReviewProvider.getProductReviewsModel
                                  .addProductReview.canAddNewReview
                              ? ProductReviewAddComponent()
                                  .getProductReviewAddComponent(
                                      context: context)
                              : Container(
                                  ...
                                  child: FlexoTextWidget().headingText15(
                                    text: localResourceProvider.getResourseByKey(
                                        "reviews.alreadyAddedProductReviews"),
                                  ),
                                ),
                          ProductReviewListComponent()
                              .getProductReviewListComponent(context: context),
                        ],
                      ),
                    ),
                  ),
