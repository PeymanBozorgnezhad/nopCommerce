...
class DownloadableProductService {
  ...
  getDownloadableProduct({
    ...
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_DOWNLOADABLE_PRODUCT_API,
  ...
  getDownloadableProductDetails(
    ...
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_DOWNLOADABLE_PRODUCT_DETAILS_API + param + param1,
        isWithoutToken: false);
  }
  ...
  getOrderItemLicense(
    ...
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_ORDER_ITEM_LICENSE_API + param,
        isWithoutToken: false);
  }

  getSampleProductProduct(
      {required BuildContext context, required String param}) async {
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_SAMPLE_PRODUCT_DETAILS_API + param,
        isWithoutToken: false);
  }

  getOrderNoteDownload(
      {required BuildContext context, required String param}) async {
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_ORDER_NOTE_FILE_API + param,
        isWithoutToken: false);
  }
}
