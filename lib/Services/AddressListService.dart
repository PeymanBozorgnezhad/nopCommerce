...
class AddressService {
  ...
  getAddresses({required BuildContext context}) async {
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_ALL_ADDRESSES_API,
  ...
  getAddressById({required BuildContext context, required String param}) async {
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_ADDRESS_BY_ID_API + param,
        isWithoutToken: false);
  }
  ...
  getAddAddresses({required BuildContext context}) async {
    return await _apiService.httpGet(
        context: context,
        url: APIConstants.GET_ADD_ADDRESS_API,
        isWithoutToken: false);
  }

  addAddress({required BuildContext context, required var body}) async {
    return await _apiService.httpPost(
        context: context, url: APIConstants.POST_ADD_ADDRESS_API, body: body);
  }
  ...
  putAddress(
    ...
    return await _apiService.httpPut(
        context: context,
        url: APIConstants.UPDATE_ADDRESS_API + param,
        body: body);
  }

  deleteAddress({required BuildContext context, required String param}) async {
    return await _apiService.httpDeleteWithoutBody(
        context: context, url: APIConstants.DELETE_ADDRESS_API + param);
  }
}
